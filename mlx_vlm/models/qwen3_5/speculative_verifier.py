...
class Qwen3_5BatchInvariantForward:
    ...
    def _attention(
        ...
        if output is None and length > 1 and not isinstance(mask, mx.array):
            # Verify block: the `length` suffix queries attend causally over the
            # full context. One SDPA call with a causal mask — equivalent to the
            # per-position slicing loop below, but 1 kernel instead of `length`,
            # and for quantized caches it avoids GBs of accumulated per-slice
            ...
            # express: keep the per-position loop.
