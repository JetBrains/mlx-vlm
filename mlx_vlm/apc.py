    ...
    def lookup_exact_cache(
        ...
        if session is not None and session_len > 0:
            built = self._build_session_cache(
                session, session_len, prompt_capacity_tokens
            )
    ...
    def store_exact_cache(
        ...
        *,
        ...
        if self.disk is not None and (
            ...
            try:
                # No stats bump here: save_exact_cache only *schedules* the
                # write, and the store's write callbacks count committed
                # files (see set_write_callbacks / _record_disk_writes).
