...
class ResponseGenerator:
    ...
    # Class-level defaults so instances built with ``__new__`` (tests, and any
    # path that skips __init__) still satisfy what _run/_step read.
    _raw_token_log = False
    draft_model = None
    draft_kind = None
    ...
    def _collect_pending_requests(
        ...
        *,
        ...
        ``--max-num-seqs`` concurrent sequences. ``max_items`` is the same kind
        of bound applied per loop iteration (used to serialize request
        processing via MLX_VLM_MAX_CONCURRENT_REQUESTS); the tighter of the two
        wins.
        """
        pending = []
        should_stop = False
        ...
        def _has_room():
            if capacity is not None and len(pending) >= capacity:
                return False
            return max_items is None or len(pending) < max_items
    ...
    def _run_impl(self):
        ...
        # Same reason as _raw_token_log above: tolerate a skipped __init__.
        if getattr(self, "_active", None) is None:
            self._active = {}
        active = self._active
        ...
        while not (self._stop and not active and self.requests.empty()):
            ...
            try:
                ...
                # Admission bound: the tighter of the serialization cap and
                # the running-batch cap.
                limit = min(
                    [
                        value
                        for value in (
                            None if concurrency_limit <= 0 else concurrency_limit,
                            max_num_seqs,
                        )
                        if value is not None
                    ],
                ...
                capacity = None if limit is None else max(0, limit - len(active))
                coalesce_s = (
