...
class TestResponseGenerator:
    ...
    @pytest.mark.parametrize(
        "draft_kind,expected_coalesce_s",
        [
            # This fork only waits to coalesce for MTP drafters, where the
            # extra batch width pays for the added first-token latency.
            ("mtp", 0.037),
            ("dflash", 0.0),
            ("eagle3", 0.0),
        ],
    ...
    def test_run_coalesces_idle_speculative_batch_generator(
        self, monkeypatch, draft_kind, expected_coalesce_s
    ):
        ...
        def fake_collect_pending_requests(
            *, active, idle_timeout=0.1, coalesce_s=0.0, capacity=None, max_items=None
        ):
        ...
        assert calls == [(False, expected_coalesce_s)]
