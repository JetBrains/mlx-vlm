...
def test_batch_apc_extra_hash_ignores_prepared_sequence_tensors():
    """The batch path never salts inputs_embeds/attention_mask.

    Unlike ``stream_generate``, which reads only caller-supplied tensors at
    entry, ``BatchGenerator``'s prompt_kwargs always carry the *prepared*
    whole-prompt tensors (the server puts them there). Those change whenever
    text is appended, so folding them into the salt would give every prompt a
    distinct namespace and no cross-request prefix could ever match. The
    tenant/image/media inputs, which are not derivable from the token ids,
    still do salt the hash.
    """
    batch_generator = SimpleNamespace(apc_manager=object())
    base = {
        "_apc_image_hash": 123,
    ...
    # The inputs that genuinely aren't recoverable from the token ids still do.
    assert (
        BatchGenerator._apc_extra_hash(batch_generator, {**base, "_apc_tenant": "b"})
        != first_hash
    )
