...
def _persist_every_exact_snapshot(monkeypatch):
    """Send unpinned exact snapshots to the disk tier for this module.

    This fork defaults APC_DISK_EXACT_SCOPE to "pinned" — only pinned
    seed-prefix stores are persisted, so live conversations stay memory-only
    and the disk cache holds a couple of seed files instead of one multi-GB
    snapshot per request. The disk-tier tests here are about write/restore
    mechanics, not that policy, and they store unpinned, so they opt into the
    persist-everything scope. Tests that assert the pinned-only default live
    in test_junie_prefix_pin.py.
    """
